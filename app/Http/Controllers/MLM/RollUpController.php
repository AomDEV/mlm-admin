...
class RollUpController extends BaseMLM
{
    ...
    public function getLogRollUp($id){
        ...
        foreach($ReferralData as $user){
            ...
            $DealerID = $this->getDealer($user->user_invite_id);
            if($DealerID <= 0 || !isset($DealerID)) $DealerID = 1;
            $UserLevel = $this->getUserLevel($user->id);
            $PercentRollUp = $this->getPercentRollUp($UserLevel);
            $RollUpResult = ($PercentRollUp / 100) * $PriceLevel;

            //dd($UserLevel, $PriceLevel, $user->product_id, $PercentRollUp, $DealerID);
            $result[] = array(
                "dealerId"=>$DealerID,
    ...
    private function getDealer($id){
        ...
        $HeaderID = isset($MyData) && isset($MyData->user_upline_id) ? $MyData->user_upline_id : 1;
