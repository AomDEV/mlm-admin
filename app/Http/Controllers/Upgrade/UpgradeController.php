...
class UpgradeController extends Controller
{
    ...
    public function checkUser(Request $request)
    {
        if ($request->has('username')) {
            // $user = UserModel::with('product')->where('username', $request->input('username'))->first();
            $user = UserModel::with('product')->where('id', $request->input('username'))->first();
            if ($user) {
    ...
    public function upgradeSave(Request $request, IndexController $indexController)
    {
        ...
        */
        ...
        if (!$validator->fails()) {
            ...
            if ($upgrade->save()) {
                // calulat point
                $user = UserModel::where('id', $upgrade->user_id)->first();
